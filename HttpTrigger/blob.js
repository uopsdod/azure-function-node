...
const ACCOUNT_ACCESS_KEY = process.env["ACCOUNT_ACCESS_KEY"];

const ONE_MEGABYTE = 1024 * 1024;
const FOUR_MEGABYTES = 4 * ONE_MEGABYTE;
const ONE_MINUTE = 60 * 1000;

async function create_or_append(aborter, containerClient, blobName, content) {
    
    // get append blob client 
    const appendBlobClient = containerClient.getAppendBlobClient(blobName);
    
    // create append blob if not exists
    const result_create = await appendBlobClient.createIfNotExists();
    if (result_create.errorCode == "BlobAlreadyExists") {
        content = "\n" + content;
    }
    const result_create_decrption = (result_create.succeeded)?" is created":" already exists";
    console.log(`blob: "${blobName}" ${result_create_decrption}`);

    // append new content to blob
    const result_append = await appendBlobClient.appendBlock(content, content.length);
    const result_append_status = result_append._response.status;
    const result_append_description = (result_append_status == 201)?"done":"append failed";
    console.log(JSON.stringify(result_append));
    console.log(`blob: "${blobName}" is appended`);

    return result_append_status;
}
...
async function execute_new(containerName, blobName, content) {
    ...
    // step01: create container 
    const containerClient = blobServiceClient.getContainerClient(containerName);
    await containerClient.createIfNotExists();
    console.log(`Container: "${containerName}" is created`);

    return await create_or_append(aborter, containerClient, blobName, content);
}

async function execute() {

    const containerName = "demo003";
